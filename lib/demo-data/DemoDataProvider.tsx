...
import { activity as initialActivity, Activity, employees, notifications as initialNotifications, Notification, projects as initialProjects, Project, tasks as initialTasks, Task, teams as initialTeams, Team } from "@/lib/demo-data/workspace";

type DemoDataContextValue = { projects: Project[]; tasks: Task[]; teams: Team[]; notifications: Notification[]; activity: Activity[]; createProject: (project: Omit<Project, "id">) => Project; updateProject: (id: string, changes: Partial<Project>) => void; deleteProject: (id: string) => void; createTask: (task: Omit<Task, "id">) => Task; updateTask: (id: string, changes: Partial<Task>) => void; deleteTask: (id: string) => void; createTeam: (team: Omit<Team, "id">) => Team; updateTeam: (id: string, changes: Partial<Team>) => void; deleteTeam: (id: string) => void; markNotificationRead: (id: string) => void };
...
export function DemoDataProvider({ children }: { children: ReactNode }) {
  ...
  return <DemoDataContext.Provider value={{ projects: projectState, tasks: taskState, teams: teamState, notifications: notificationState, activity: initialActivity, createProject, updateProject, deleteProject, createTask, updateTask, deleteTask, createTeam, updateTeam, deleteTeam, markNotificationRead }}>{children}</DemoDataContext.Provider>;
}
